...
//                Log.i("MYTAG", snapshot.child("pole_positions").getValue(Integer.class).toString());
                ...
                Glide.with(ConstructorDetailsActivity.this).load(snapshot.child("logo").getValue(String.class)).into(logo);
                //}
            }

            @Override
            public void onCancelled(@NonNull DatabaseError error) {
//                Toast.makeText(ConstructorDetailsActivity.this,error.getMessage(), Toast.LENGTH_LONG).show();
//                Snackbar.make(, "Error", Snackbar.LENGTH_LONG).show();
            }
        });
    }
}